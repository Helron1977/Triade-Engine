 ...
 * Impl├®mentation robuste avec Bounce-Back int├®gr├® au Streaming.
 */
export class AerodynamicsEngine implements IHypercubeEngine {
    ...
    /**
     * Initialisation WebGPU : Pr├®pare les pipelines et les bindings.
     */
    /**
     * Initialisation sp├®cifique au GPU. Pr├®pare les pipelines et les bindings.
     */
    ...
    public computeGPU(device: GPUDevice, commandEncoder: GPUCommandEncoder, nx: number, ny: number, nz: number): void {
        ...
        // --- PASS 2: Vorticit├® (D├®pend de Pass 1) ---
        ...
        for (let lz = 0; lz < nz; lz++) {
            ...
            for (let y = 1; y < ny - 1; y++) {
                for (let x = 1; x < nx - 1; x++) {
                    ...
                    // Appliquer l'inlet sur la bordure gauche avec un petit bruit pour casser la sym├®trie
